...
面向 −30°C 环境的5 kW净输出燃料电池电源研究项目。当前V2采用纯气态储氢，开展电堆、辅机、散热和氢量的联合计算；历史双储氢方案与R12装配保留为研究记录。

**进度快照：2026-09-23 · 当前计算：V2纯气态储氢 · 历史装配：R12。**
现有结果未证明已达到 5 kW 净输出、120 s 低温启动或寿命指标。

## 最新：V2纯气态储氢联合计算

- [第二版设计与稳态联合求解](designs/v2-gaseous/第二版设计.md)
- [−30℃暖堆稳态、冷液水力与启动热量计算](designs/v2-gaseous/第二版设计_负30度补充计算.md)
- [计算入口、依赖与校验方法](designs/v2-gaseous/README.md)

在暂定4000 m、35℃、4 h任务下，5 kW净输出的基准解需要约6.20 kW堆毛功率、5.20 kW液冷排热及66.2 L气瓶水容积；布局先按2×40 L预留。固定散热器在−30℃环境且电堆已升温时，计入局部伴热及风机预算后毛功率约5.72 kW。上述均为带假设的计算，不是供应商保证或实测；冷浸启动尚未验证。

多项不利假设组合需要约8.15 kW、97.85 A，超过参考7.1 kW/80 A电堆能力，已标记不可行。历史R12 CAD并未按V2重建，不能用于证明V2尺寸。
...
| 时间（北京时间） | 思考与决策 | 搜索、计算或校核带来的结果 |
|---|---|---|
| 7 月 18–22 日 | 比较储氢路线，厘清混合能源与冷启动 | 区分局部 CFD 与整机控制；粗算中冷器温升、分水器产水量 |
| 7 月 27 日–8 月 17 日 | 图纸应表达真实连接与控制关系 | P&ID 连通/排版检查；低压电气引脚、CAN 和硬线急停审查 |
| 8 月 30 日–9 月 6 日 | 从容量、耗氧、余热推导零件尺寸 | 空气加热负荷和光管体积被重新修正；发现续航与轻量化冲突 |
| 9 月 9 日 | 查论文、专利和器件，形成完整概念设计 | 70 节方案、质量/能量模型与 FreeCAD 概念装配 |
| 9 月 10–11 日 | 改固态∶气态为 6∶4，压缩布局 | R10→R12；几何检查通过，但 R12 仍有 72 个工具空间受限接口 |
| 9 月 12–14 日 | 检查保温冷启动、重量及报告参数自洽 | 45 min 与 120 s 的定义冲突；区分 70/53/55 节及不同容量版本 |
| 9 月 15 日 | 阀门不能只按电气支线计数 | 逐项核对 14 个对象、52 个外部流体接口 |
| 9 月 19 日 | 公开进展与研究过程 | 重跑概念计算、发布模型及证据，保留未验证项 |

[阅读完整时间线：你的提问与选择、检索来源、计算结果和后续修正](docs/TIMELINE.md)。历史讨论结果按当时工况记录，不等同于实测性能。
...
- [ ] 取得参考电堆高原极化、低温启动和冷却液许可数据。
- [ ] 用泵、风机、散热器及电池实测曲线替换V2假设。
- [ ] 完成纯气态储氢P&ID、接口表和真实器件CAD。
